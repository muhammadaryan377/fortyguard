...
def analyze_spatial_features(
    ...
    *,
    ...
    indexed = [
        ...
        if (item := _valid_tile(feature, index, request)) is not None
    ]
    ...
    base = dict(
        ...
        tiles=tiles,
        limitations=SPATIAL_LIMITATIONS,
    )
    ...
    if not containing:
        return SpatialHeatResponse(
            ...
            summary=SpatialHeatSummary(
                ...
                coolest_mapped_temperature_c=min(
                    (tile.temperature_c for tile in tiles),
                    default=None,
                ),
            ),
            **base,
    ...
    eligible = [
        ...
        if index != site_index
        and not tile.contains_site
        and tile.temperature_c < site_tile.temperature_c
    ]
    eligible.sort(
        key=lambda item: (
            item[1].temperature_c,
            item[1].straight_line_distance_m,
            item[0],
        )
    )
    candidates = [
        CoolerZoneCandidate(
            ...
            cooler_by_c=site_tile.temperature_c - tile.temperature_c,
            centroid_latitude=tile.centroid_latitude,
            centroid_longitude=tile.centroid_longitude,
            straight_line_distance_m=tile.straight_line_distance_m,
            polygon_coordinates=tile.polygon_coordinates,
            rank=rank,
            limitations=CANDIDATE_LIMITATIONS,
        )
        for rank, (_index, tile) in enumerate(eligible[: request.max_candidates], 1)
    ]
    summary = SpatialHeatSummary(
        valid_tile_count=len(tiles),
        cooler_candidate_count=len(candidates),
        coolest_mapped_temperature_c=min(tile.temperature_c for tile in tiles),
        coolest_candidate_cooler_by_c=max(
            (candidate.cooler_by_c for candidate in candidates),
            default=None,
        ),
        nearest_cooler_candidate_distance_m=min(
            (candidate.straight_line_distance_m for candidate in candidates),
            default=None,
        ),
    )
    return SpatialHeatResponse(
    ...
    *,
    ...
    for granularity in _granularity_candidates(int(request.granularity)):
        ...
        heatmap_request = HeatmapRequest(
            polygon_aoi=build_site_polygon(
                request.location.latitude,
                request.location.longitude,
