...
export default function TodayScreen({ location, locationBusy, fortyGuardSupported, cycle, weather, weatherBusy, analysisBusy, operationBusy, work, onRefresh, onAnalyze, onNavigate }) {
  ...
  const workspace = useMemo(() => {
    ...
    const siteId = loadSelectedSiteId(sites);
    const site = sites.find((item) => item.id === siteId) ?? sites[0] ?? null;
    const crew = site ? (loadCrewMap()[site.id] ?? []) : [];
    return { site, crew, plan: loadOperationalSummary(site, crew) };
  }, [location]);
  const planWorker = workspace.plan?.workers?.[0] ?? null;
  const cycleEnv = cycle?.current_assessment?.environmental_evidence ?? {};
  const env = planWorker?.environment ?? cycleEnv;
  const band = planWorker?.band ?? cycle?.current_assessment?.screening?.band;
  ...
  const cycleActions = cycle?.agent_decision?.actions ?? [];
  const plannedAction = planWorker?.primaryAction ? { action_type: planWorker.primaryAction.actionType, status: planWorker.primaryAction.status } : null;
  const action = plannedAction ?? cycleActions.find((item) => item.status === "proposed") ?? cycleActions[0] ?? null;
  const actionCopy = action ? ACTION_COPY[action.action_type] : null;
  const savedWorker = workspace.crew[0] ?? null;
  const worker = cycle?.current_assessment?.worker_context ?? {};
  const task = cycle?.current_assessment?.task_context ?? {};
  const hasRealWorker = Boolean(workspace.crew.length || planWorker || worker.display_name || worker.name || worker.worker_name || (worker.worker_id && worker.worker_id !== "WORKER-01"));
  const displayName = planWorker?.name || savedWorker?.name || savedWorker?.workerId || (hasRealWorker ? (worker.display_name || worker.name || worker.worker_name || worker.worker_id) : null);
  const taskName = planWorker?.taskName || savedWorker?.currentTask || task.task_name || work?.taskName || "Task not configured";
  ...
  const locationName = workspace.site?.name || location?.site_name || location?.name || location?.city || (locationBusy ? "Finding location…" : "Select worksite");
  const refresh = () => cycle?.cycle_id ? onRefresh?.() : onAnalyze?.();
  const workerCount = workspace.crew.length || workspace.plan?.workers?.length || (hasRealWorker ? 1 : 0);

  return <div className="hs-screen hs-today-overview">
    <button className="hs-today-location" type="button" onClick={() => onNavigate("map")}>
      <span className="hs-location-pin"><MapPin/></span><span className="hs-location-copy"><strong>{locationName}</strong><small>{timezoneLabel(location)}</small><em><MapPin/> {workspace.site?.address || location?.display_name || [location?.city, location?.state, location?.country].filter(Boolean).join(", ")}</em></span><span className="hs-change-location">Open map <Pencil/></span>
    ...
    <section className="hs-overview-card">
      <div className="hs-card-title"><h1>Heat Risk Overview</h1><span>{planWorker ? `${planWorker.name} · highest attention worker` : "Current Conditions"}</span></div>{workspace.plan ? <span className="hs-replay-badge">ACTIVE PLAN</span> : replay ? <span className="hs-replay-badge">VERIFIED REPLAY</span> : null}
    ...
    <section className="hs-compact-card hs-workers-panel"><div className="hs-card-title hs-panel-heading"><h2>Active Workers {workerCount ? `· ${workerCount}` : ""}</h2>{hasRealWorker ? <button type="button" onClick={() => onNavigate("map")}>View on map</button> : null}</div>{hasRealWorker ? <button className="hs-detail-row" type="button" onClick={() => onNavigate(workspace.plan ? "plan" : "map")}><span className="hs-worker-avatar">{initials}</span><span className="hs-detail-copy"><strong>{displayName}</strong><small>{taskName}{planWorker?.zoneLabel ? ` · ${planWorker.zoneLabel}` : ""}</small></span><span className={`hs-worker-badge risk-${band || "unknown"}`}>{riskLabel}</span><ChevronRight/></button> : <div className="hs-worker-empty"><UsersIcon/><strong>No active workers yet</strong><span>Add workers on the site map, then generate worker plans.</span><button type="button" onClick={() => onNavigate("map")}>Add workers</button></div>}</section>
    <section className="hs-compact-card hs-plans-panel"><div className="hs-card-title hs-panel-heading"><h2>Worker Plans {workspace.plan?.workers?.length ? `· ${workspace.plan.workers.length}` : ""}</h2>{workspace.plan ? <button type="button" onClick={() => onNavigate("plan")}>View all plans</button> : null}</div>{workspace.plan && planWorker ? <button className="hs-detail-row" type="button" onClick={() => onNavigate("plan")}><span className="hs-action-icon"><ShieldCheck/></span><span className="hs-detail-copy"><strong>{actionCopy?.title || (action ? humanize(action.action_type) : "Plan ready for supervisor review")}</strong><small>{planWorker.name} · {action?.status === "proposed" ? "supervisor review required" : humanize(planWorker.agentStatus || "plan ready")}</small></span><ChevronRight/></button> : <div className="hs-worker-empty"><ShieldCheck/><strong>No worker plans yet</strong><span>Generate the provider-backed plan after site and workers are ready.</span><button type="button" onClick={() => onNavigate("plan")}>Generate plans</button></div>}</section>
