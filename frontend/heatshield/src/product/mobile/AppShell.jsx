...
import { AlertTriangle, Bell, CalendarDays, ChevronLeft, ChevronRight, CircleHelp, FileText, History, MapPinned, RefreshCw, Settings, Shield, Users, X } from "lucide-react";
import HeatHistoryScreen from "./HeatHistoryScreen.jsx";

const TABS = [["today", "Today", Shield], ["map", "Map", MapPinned], ["history", "Heat History", History], ["plan", "Plan", CalendarDays], ["team", "Team", Users], ["alerts", "Alerts", Bell]];
...
export default function AppShell({ children, activeTab, onNavigate, location, cycle, message, error, onDismissMessage, onDismissError, onRefresh, analysisBusy, operationBusy }) {
  ...
  return <div className="hs-page"><div className={`hs-app-frame hs-desktop-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <div className="hs-shell-main">
      ...
      <main className="hs-content">{error ? <div className="hs-toast hs-toast-error"><AlertTriangle/><span>{error}</span><button type="button" onClick={onDismissError} aria-label="Dismiss error"><X/></button></div> : null}{message ? <div className="hs-toast hs-toast-success"><span>{message}</span><button type="button" onClick={onDismissMessage} aria-label="Dismiss message"><X/></button></div> : null}{activeTab === "history" ? <HeatHistoryScreen location={location}/> : children}</main>
      <nav className="hs-bottom-nav" style={{ gridTemplateColumns: `repeat(${TABS.length}, 1fr)` }} aria-label="HeatShield navigation">{TABS.map(([id,label,Icon]) => <button key={id} type="button" className={navActive === id ? "active" : ""} onClick={() => onNavigate(id)}><Icon/><span>{label}</span></button>)}</nav>
    </div>
  </div></div>;
}
